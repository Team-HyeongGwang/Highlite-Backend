...
- [이미지/도표]: 그림이나 도표 안에 포함된 글자들을 따로 떼어내지 마세요. 도표의 의미와 포함된 텍스트를 종합하여 객관적인 묘사로 `content`에 작성합니다. 도표/이미지에 강조 표시가 있으면 color 필드에만 기록합니다.
- [형광펜]: 형광펜으로 강조된 텍스트가 있으면 반드시 highlight_color 필드에 색상을 명시하세요.
