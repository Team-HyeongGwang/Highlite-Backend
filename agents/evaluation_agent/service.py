...
0-1. [즉시 반려] 아래 패턴 중 하나라도 해당하면 is_approved=false, quality_score=1로 설정하세요:
   ...
   - 선지가 '원문에서 언급된 ~이다', '원문에서 ~로 묘사되었다' 등 원문 존재 여부만 묻는 형태
   - '[필기: ...]' 태그 형식 자체나 과목명을 빈칸으로 만든 경우
   - 선지에 '①이 표시되어 있다', '③이 표시되어 있다' 등 원문의 번호/기호 표시가 그대로 노출된 경우
   - 실질적인 학습 개념(정의, 특징, 원리, 적용)이 아닌 메타 정보나 형식 정보를 묻는 문제
